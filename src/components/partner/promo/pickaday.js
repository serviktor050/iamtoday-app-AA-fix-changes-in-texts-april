...
(function(root, factory) {
  ...
  let renderRow = function(days, isRTL) {
  ...
  let renderTitle = function(instance, c, year, month, refYear) {
    var i
    var j
    var arr
  ...
  let renderTable = function(opts, data) {
    return '<table cellpadding="0" cellspacing="0" class="pika-table">' +
      renderHead(opts) +
      renderBody(data) +
      '</table>'
  }
  let renderTimePicker = function(
    numOptions,
    selectedVal,
    selectClass,
    displayFunc,
    ...
    for (var i = 0; i < numOptions; i += incrementBy) {
      toReturn += '<option ' +
        (disabled && disabled(selectedVal) ? 'disabled' : '') +
        ' value="' +
        i +
        '" ' +
  ...
  let renderTime = function(hh, mm, ss, opts, self) {
      ...
      if (hasMoment) {
        ...
        if (date) {
          if (self.isTime(opts.field.value)) {
            var defaultDate = moment(opts.defaultDate)
            date = date.set({
  ...
  Pikaday.prototype = {
    ...
    render: function(year, month) {
      ...
      var daysInPreviousMonth = getDaysInMonth(
        yearOfPreviousMonth,
        previousMonth
      )
      ...
      for (var i = 0, r = 0; i < cells; i++) {
        ...
        var isInRange = opts.startRange &&
          opts.endRange &&
          opts.startRange < day &&
          day < opts.endRange
        var isDisabled = (opts.minDate && day < minDateDate) ||
          (opts.maxDate && day > maxDateDate) ||
