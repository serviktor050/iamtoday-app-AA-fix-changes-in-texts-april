...
class LoginFB extends Component {
  componentDidMount() {
    ...
    const queryMatch = queryHash.match(/#access_token=(.*)&exp.*/)
    token = queryMatch && queryMatch[1]
  ...
  render() {
    ...
    return (
      <div>
        { paymentType === 'err' ? <LoadingView title="Если вы видите это окно, значит мы делаем личный кабинет для вас удобнее! Напишите нам в чат тех поддержки (внизу справа) или на почту av@todayme.ru"/> : <div>
...
export default CSSModules(LoginFB, styles)
