export const OKSDK = (function() {
  ...
  function init(args, success, failure) {
    ...
    sdkSuccess = isFunc(success) ? success : nop
    sdkFailure = isFunc(failure) ? failure : nop
    ...
    if (!state.app_id || !state.app_key) {
      sdkFailure('Required arguments app_id/app_key not passed')
      return
    }
    ...
    if (!params['api_server']) {
      ...
      if (hParams['error'] != null) {
        sdkFailure('Error with OAUTH authorization: ' + hParams['error'])
        return
      }
    }
    sdkSuccess()
  ...
  function restCall(method, params, callback, callOpts) {
    ...
    var callbackId = '__oksdk__callback_' + (++restCounter)
  ...
  function widgetBackButton(onSuccess, style) {
    ...
    restCall('widget.getWidgetContent', {
      wid: state.header_widget || 'mobile-header-small',
      style: style || null
    },
