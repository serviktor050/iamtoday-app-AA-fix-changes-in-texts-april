...
import styles from './seasonSignupParams.css'
...
/**
 *  Контейнер SeasonSignupParams.
 *  Используется для отображения страницы 'Сезон' (/season)
 */
class SeasonSignupParams extends Component {
  /**
   * @memberof SeasonSignupParams
   ...
   * @prop {func} propTypes.fetchProgramsIfNeeded Функция для получения данных с сервера для рендеринга страницы 'Сезон'.
   *
   * */
  static propTypes = {
    ...
    /**
     * Функция для получения данных с сервера для рендеринга страницы 'Сезон'.
     * @memberof SeasonSignupParams
     * @param {number} programs - Программы.
     */
    fetchProgramsIfNeeded: PropTypes.func.isRequired,
  ...
  componentDidMount() {
    const { dispatch, selectedPrograms, choosenPackageType, choosenPromo } = this.props
  ...
  render() {
    let { dispatch, packageType, share, choosenPackageType, choosenAmount,
      choosenProgram, choosenPromo, receivePayment, choosenTomorrowType, fetchedPrograms } = this.props
    ...
    //TODO: fix this later
    if (choosenProgram !== 0 && choosenProgram > (programsData.hero-1)) {
      programs[choosenProgram - programsData.hero].isActive = true
    }
    ...
    const paymentCreate = () => {
      ...
      .then(json => {
        if (json.isSuccess && json.data) {
          ...
          if (choosenTomorrowType === 1 && choosenProgram === 16) {
      ...
      <div className={styles.layoutEntry}>
        ...
        <div className={styles.entryMin}>
          <div className={styles.entryInner}>
            <div className={`entry__header entry__header--colorful g-${headerStyle}`}>
              <h2 className={styles.entryTitleCenter} style={{ color: '#fff' }}>
                {/* TODO: fix this later */}
                {choosenProgram !== 0 && choosenProgram > 12 ? programs[choosenProgram - programsData.hero].name : ''}
              </h2>
            </div>
            <br />
              ...
              {!cookie.load('program') &&
                <ul className={styles.optionsWhiteWideMb30}>
                  {programs.map((p, index) => (
                    <li key={index}
                      //className={`options__item g-${p.str} ${p.isActive && 'is-active'}`}
                      className = {p.style}
                      onClick={() => {
                ...
                <ul className={styles.optionsWhiteThreeValMt30}>
                  {packageTypes.map((pt, index) => {
                      ...
                      if(pt.half){
                        ...
                        return (
                          <li className={styles.optionsItemHalfWrap} style ={pt.number === 2 ? {paddingRight: '5px'} : {paddingLeft: '5px'}}>
            ...
            <div className={styles.entryBox}>
              <p className={styles.entryProgramPriceMb10}>{choosenAmount.price} руб.</p>
              ...
              <div className={styles.buttonPrimaryWide} onClick={() => {
                return fetch(`${api}/day/package-get`, {
                  ...
                  method: 'POST',
                  body: JSON.stringify({ promoName: choosenPromo })
                })
                .then(response => response.json())
                .then(json => {
...
const mapStateToProps = state => {
  ...
  const { selectedPrograms, recivedPrograms, profile, choosenPackageType, choosenAmount, choosenProgram, choosenPromo, choosenTomorrowType } = state
...
SeasonSignupParams = connect(
  mapStateToProps,
  mapDispatchToProps
)(SeasonSignupParams)

export default CSSModules(SeasonSignupParams, styles)
