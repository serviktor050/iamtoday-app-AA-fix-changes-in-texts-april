...
class BlockedProfiles extends Component {
  ...
  renderHeader() {
    ...
    const items = [
      i18n["admin.main"],
      i18n["admin.profiles"],
      i18n["admin.profiles.blocked"],
    ];
    const links = ["/admin/profiles", "/admin/profiles"];
    return (
      <div className={cx(styles.profileHeader)}>
        <h2 className={styles.profileHeader__title}>Заблокированные анкеты</h2>
...
BlockedProfiles = connect(mapStateToProps, mapDispatchToProps)(BlockedProfiles);

export default CSSModules(BlockedProfiles, styles);
