...
/**
 *  Компонент DeclinedProfiles.
 *  Используется для отображения страницы админки
 *
 */
class DeclinedProfiles extends Component {
  /**
   * @memberof DeclinedProfiles
  ...
  renderHeader() {
    ...
    const items = [
      i18n["admin.main"],
      i18n["admin.profiles"],
      i18n["admin.profiles.declined"],
    ];
    const links = ["/admin/profiles", "/admin/profiles"];
    return (
      <div className={cx(styles.profileHeader)}>
        <h2 className={styles.profileHeader__title}>Отклоненные анкеты</h2>
...
DeclinedProfiles = connect(mapStateToProps, mapDispatchToProps)(DeclinedProfiles);

export default CSSModules(DeclinedProfiles, styles);
