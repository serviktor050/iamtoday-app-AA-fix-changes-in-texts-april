...
/**
 *  Компонент NewProfile.
 *  Используется для отображения страницы админки
 *
 */
class NewProfiles extends Component {
  /**
   * @memberof NewProfiles
      ...
      for (let index = 0; index < user.userInfoDocuments.length; index++) {
        ...
        if (item) {
          userInfoDocuments.push({
            ...
            verificationStatus: item.verificationStatus === "1" ? 2 : status,
          });
        } else {
  ...
  renderHeader() {
    ...
    const items = [
      i18n["admin.main"],
      i18n["admin.profiles"],
      i18n["admin.profiles.new"],
    ];
    const links = ["/admin/profiles", "/admin/profiles"];
    return (
      <div className={cx(styles.profileHeader)}>
        <h2 className={styles.profileHeader__title}>Новые анкеты</h2>
...
NewProfiles = connect(mapStateToProps, mapDispatchToProps)(NewProfiles);

export default CSSModules(NewProfiles, styles);
