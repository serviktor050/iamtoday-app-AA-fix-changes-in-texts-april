    ...
    render() {
        ...
        return (
            <div className={styles.questionsList}>
                {this.renderHeader()}
                <ul>
                    {Boolean(allQuestions.length) && allQuestions.map((item) => {
        ...
        return (
            <div>
                <div className={styles.questionsList_title}>
                    ...
                    {isAdmin && <button className={styles.questionsList_titleButton} onClick={this.openAddQuestionBlock}>+</button>}
                </div>
                {isAddBlockOn &&
                    <AddQuestionBlock
            ...
            <li className={styles.questionItem} key={item.id} value={item.id} >
                <div>
                    <div className={styles.questionItem_header} >
                        {Boolean(item.tags) && Boolean(item.tags.length) && this.renderTagsList(item.tags)}
                        <button className={styles.questionItem_arrow}
                            //arrow button rotate 180deg when question is expanded
                            style={{ transform: expandedQuestionId === item.id && isExpanded && "rotate(180deg)" }}
                            onClick={() => this.expandBlock(item.id)}></button>
                    </div>
                </div>
                <div className={styles.questionText}>
                    {item.question}
                    {isAdmin &&
                        <span className={styles.questionText_edit} onClick={() => this.openEditQuestionBlock(item.id)} >
                            <svg className={styles.svgIconFile}>
                                <use xlinkHref="#ico-pencil" />
                            </svg>
                        </span>
                ...
                {expandedQuestionId === item.id && isExpanded ? <div className={styles.questionAnswer}>{item.answer}</div> : null}
            </li>
        )
    }
