...
import { getFaqTagsAll } from "../../../../utils/api";
...
class TagsListBlock extends Component {
  ...
  componentDidMount() {
    getFaqPopularTags().then((res) => {
      this.setState({ popularTagsList: res.data.data, take: 10 });
    });
  }

  handleMoreTagsClick = () => {
    getFaqTagsAll().then((res) => {
      this.setState({ popularTagsList: res.data.data });
    });
  };
  ...
  render() {
    ...
    return (
      <div className={styles.tagsList}>
        ...
        {popularTagsList.length === 10 && (
          <button
            className={styles.moreTagButton}
            onClick={this.handleMoreTagsClick}
          >
