...
class FAQ extends Component {
  ...
  componentDidMount() {
    getFaqCategory().then((res) => {
      this.setState({
        categoriesList: res.data.data,
        chosenCategory: res.data.data[0],
      });
  ...
  getCategoryQuestions = (id) => {
    getFaqQuestions({ categoryId: id }).then((res) =>
      this.setState({
        questionsList: res.data.data,
        totalQuestions: res.data.itemsCounter,
      })
    );
  };
  ...
  handleFilterChange = (e) => {
    ...
    getFaqCategory({ question: newSearchFilter }).then((res) => {
